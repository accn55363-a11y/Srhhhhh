...
# ==================== قراءة إعدادات التليجرام الخارجية ====================
TELEGRAM_TOKENN = ""
try:
  if os.path.exists("config.json"):
    with open("config.json", "r", encoding="utf-8") as f:
      conf = json.load(f)
      TELEGRAM_TOKENN = conf.get("telegram_tokenn", "").strip()
  else:
    # محاولة جلبه من متغيرات البيئة في حال الاستضافة السحابية
    TELEGRAM_TOKEN = os.environ.get("TELEGRAM_TOKENN", "").strip()
...
bot = telebot.TeleBot(TELEGRAM_TOKENN)

# ==================== إعدادات فري فاير (الشفرات) ====================
...
@bot.message_handler(commands=["samrrerr", "he"])
def send_welcome(message):
  markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
  markup.add(
      types.KeyboardButton("🚀 بدء السبام"), types.KeyboardButton("🛑 إيقاف السبام")
  )
  markup.add(
      types.KeyboardButton("📊 حالة السبام"), types.KeyboardButton("👥 الحسابات")
  )
  markup.add(types.KeyboardButton("⚡ فحص البينغ"))
  help_text = (
      "🔥 **أهلاً بك في بوت سبام طلبات الصداقة (مقبرة skip)** 👾\n\n"
      "📋 **قائمة الأوامر المتاحة:**\n"
      "⚡ `/sp <UID>` - لبدء إرسال طلبات الصداقة للايدي\n"
      "🛑 `/st <UID>` - لإيقاف السبام عن ايدي معين\n"
      "📊 `/ss` - لعرض الأيديات النشطة حالياً\n"
      "👥 `/ac` - لعرض إحصائيات الحسابات (المتصلة وغير المتصلة)\n"
      "🏓 `/pg` - لفحص سرعة استجابة السيرفر\n"
      "❓ `/he` - لعرض هذه الرسالة"
  )
  bot.reply_to(message, help_text, reply_markup=markup, parse_mode="Markdown")


@bot.message_handler(commands=["pg"])
def cmd_ping(message):
  bot.reply_to(
      message,
      "⚡ **السيرفر يعمل بأقصى قوة وسرعة وثبات!** 🚀",
      parse_mode="Markdown",
  )
...
def cmd_accounts(message):
  ...
  text = (
      ...
      f"📂 إجمالي الحسابات بملف `accs.txt`: `{total_accs}`\n"
      f"🟢 الحسابات المتصلة (النشطة حالياً): `{connected_accs}`\n"
...
def cmd_status(message):
  if not aCtive:
    bot.reply_to(
        message,
        "💤 **لا توجد أي عمليات سبام نشطة في الوقت الحالي.**",
        parse_mode="Markdown",
    )
    return

  text = "🎯 **الأيديات التي يتم استهدافها بالسبام حالياً:**\n\n"
...
def cmd_spam(message):
  ...
  if len(parts) < 2:
    msg = bot.reply_to(
        message, "🎯 **الرجاء إرسال الآيدي (UID) المستهدف الآن للبدء:**"
    )
    bot.register_next_step_handler(msg, process_inline_spam)
    return
...
@bot.message_handler(func=lambda message: message.text == "🚀 بدء السبام")
def btn_start_spam(message):
  msg = bot.reply_to(
      message, "🎯 **الرجاء إرسال الآيدي (UID) المستهدف لبدء السبام:**"
  )
  bot.register_next_step_handler(msg, process_inline_spam)


def process_inline_spam(message):
...
def start_spam_process(message, uid):
  if not uid.isdigit():
    bot.reply_to(
        message, "❌ **خطأ:** الآيدي يجب أن يكون يتكون من أرقام صحيحة فقط!"
    )
    return
  ...
  if uid in aCtive:
    bot.reply_to(
        message,
        f"⚠️ **تنبيه:** الآيدي `{uid}` يتم استهدافه بالسبام بالفعل مسبقاً!",
        parse_mode="Markdown",
    )
    return
...
def cmd_stop(message):
  ...
  if len(parts) < 2:
    msg = bot.reply_to(
        message, "🛑 **الرجاء إرسال الآيدي (UID) المراد إيقاف السبام عنه:**"
    )
    bot.register_next_step_handler(msg, process_inline_stop)
    return
...
@bot.message_handler(func=lambda message: message.text == "🛑 إيقاف السبام")
def btn_stop_spam(message):
  msg = bot.reply_to(
      message, "🛑 **الرجاء إرسال الآيدي (UID) المراد إيقاف السبام عنه:**"
  )
  bot.register_next_step_handler(msg, process_inline_stop)
...
def process_inline_stop(message):
  uid = message.text.strip()
  stop_spam_process(message, uid)
...
def stop_spam_process(message, uid):
  if not uid.isdigit():
    bot.reply_to(
        message, "❌ **خطأ:** الآيدي يجب أن يتكون من أرقام صحيحة فقط!"
    )
    return
  ...
  if uid in aCtive:
    ...
    bot.reply_to(
        message,
        f"⚠️ **تنبيه:** الآيدي `{uid}` ليس قيد السبام أصلاً.",
        parse_mode="Markdown",
    )


@bot.message_handler(func=lambda message: message.text == "📊 حالة السبام")
def btn_status(message):
  cmd_status(message)


@bot.message_handler(func=lambda message: message.text == "👥 الحسابات")
def btn_accounts(message):
  cmd_accounts(message)


@bot.message_handler(func=lambda message: message.text == "⚡ فحص البينغ")
def btn_ping(message):
  cmd_ping(message)


# ==================== إعدادات مسارات الـ Flask ====================
app = Flask(__name__)
...
def run_telegram_bot():
  print("Telegram Bot is running with skip branding...")
  bot.infinity_polling()


if __name__ == "__main__":
  if not TELEGRAM_TOKENN:
    print("❌ خطأ: التوكن غير موجود. تأكد من ضبط الإعدادات قبل التشغيل.")
  else:
